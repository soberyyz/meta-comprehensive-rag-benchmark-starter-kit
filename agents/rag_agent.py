...
class SimpleRAGAgent(BaseAgent):
    ...
    def generate_response(self, query: str, image: Optional[str] = None, conversation_history: Optional[List[Dict[str, str]]] = None) -> str:
        ...
        summarize_input_text = self.processor.apply_chat_template(summarize_messages, add_generation_prompt=True)
        ...
        summarize_output = self.model.generate(**summarize_inputs, max_new_tokens=self.max_gen_len)
        summarize_answer = self.processor.decode(summarize_output[0])
        summarize_answer = summarize_answer.split('<|start_header_id|>assistant<|end_header_id|>')[-1].split('<|eot_id|>')[0]
        ...
        rag_prompt = "Here are some other relevant images and information that may help you.\n\n"
