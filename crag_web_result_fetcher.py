...
class WebSearchResult:
    ...
    def _get_cache_filename(self) -> str:
        return os.path.join(CACHE_DIR, sha256(self.result["url"].encode()).hexdigest())
    ...
    def _fetch_page_content(self) -> str:
        response = requests.get(self.result["url"])
        response.raise_for_status()
        content = response.text
        with open(self._get_cache_filename(), "w") as f:
            f.write(content)
        self.result["page_content"] = content
        return content

    def _get_page_content_from_cache(self) -> str:
        with open(self._get_cache_filename(), "r") as f:
            return f.read()
